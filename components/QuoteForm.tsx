...
const QuoteForm: React.FC<QuoteFormProps> = ({ isOpen, onClose, onSave, clients, interventionTemplates, parts, existingQuote, nextQuoteNumber }) => {
  ...
  const handlePartQuantityChange = (laborIndex: number, partIndex: number, quantity: number) => {
    const newItems = [...quote.laborItems];
    newItems[laborIndex].partItems[partIndex].quantity = quantity;
    setQuote(prev => ({...prev, laborItems: newItems}));
  };
  ...
  const addPartItem = (laborIndex: number) => {
      const newItems = [...quote.laborItems];
      newItems[laborIndex].partItems.push({...emptyPartItem, id: crypto.randomUUID()});
      setQuote(prev => ({...prev, laborItems: newItems}));
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const quoteToSave: Quote = {
      ...
      statusHistory: existingQuote?.statusHistory || [{ status: 'draft', date: new Date().toISOString() }],
      ...quote,
      laborItems: quote.laborItems.filter(l => l.description).map(l => ({
          ...l,
          partItems: l.partItems.filter(p => p.partId)
      })),
    };
    onSave(quoteToSave);
  };
                         ...
                         {labor.partItems.map((part, pIndex) => (
                            <div key={part.id} className="grid grid-cols-12 gap-2 items-center">
                                <div className="col-span-12 sm:col-span-8">
                                    <select 
                                        value={part.partId} 
                                        onChange={e => handlePartSelect(lIndex, pIndex, e.target.value)}
                                ...
                                <div className="col-span-4 sm:col-span-2"><input type="number" placeholder="Qté" value={part.quantity} onChange={e => handlePartQuantityChange(lIndex, pIndex, parseInt(e.target.value, 10) || 0)} className="w-full text-sm p-1 rounded-md bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600"/></div>
                                <div className="col-span-4 sm:col-span-2 text-right"><button type="button" onClick={() => removePartItem(lIndex, pIndex)} className="text-red-500"><TrashIcon className="h-4 w-4"/></button></div>
                            </div>
                         ))}
                        <button type="button" onClick={() => addPartItem(lIndex)} className="text-sm flex items-center gap-1 text-green-600 dark:text-green-400 font-semibold"><PlusIcon className="h-4 w-4"/> Pièce</button>
                    </div>
                </div>
            ))}
