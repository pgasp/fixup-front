            ...
            {filteredParts.length > 0 ? filteredParts.map(part => (
              <tr key={part.id} className={`${part.stock <= 5 ? 'bg-red-50 dark:bg-red-900/20' : ''} hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors`}>
                ...
                <td className="p-3 whitespace-nowrap text-center">
                  ...
                  <button onClick={() => onDelete(part.id)} className="p-2 text-gray-500 hover:text-red-500 dark:hover:text-red-400" title="Supprimer"><TrashIcon className="h-5 w-5"/></button>
                </td>
              </tr>
            )) : (
                <tr>
